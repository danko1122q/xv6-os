...
void iappend(uint inum, void *xp, int n) {
    ...
    while (n > 0) {
        ...
        if (fbn < NDIRECT) {
            if (xint(din.data.addrs[fbn]) == 0) { // PERBAIKAN: din.data.addrs
                din.data.addrs[fbn] = xint(freeblock++);
            }
            x = xint(din.data.addrs[fbn]);
        } else if (fbn < NDIRECT + NINDIRECT) {
            if (xint(din.data.addrs[NDIRECT]) == 0) { // PERBAIKAN: din.data.addrs
                din.data.addrs[NDIRECT] = xint(freeblock++);
            }
            rsect(xint(din.data.addrs[NDIRECT]), (char *)indirect);
            if (indirect[fbn - NDIRECT] == 0) {
                indirect[fbn - NDIRECT] = xint(freeblock++);
                wsect(xint(din.data.addrs[NDIRECT]), (char *)indirect);
            }
            x = xint(indirect[fbn - NDIRECT]);
        } else {
            fbn -= (NDIRECT + NINDIRECT);
            if (xint(din.data.addrs[NDIRECT + 1]) == 0) { // PERBAIKAN: din.data.addrs
                din.data.addrs[NDIRECT + 1] = xint(freeblock++);
            }
            rsect(xint(din.data.addrs[NDIRECT + 1]), (char *)indirect);
            if (indirect[fbn / NINDIRECT] == 0) {
                indirect[fbn / NINDIRECT] = xint(freeblock++);
                wsect(xint(din.data.addrs[NDIRECT + 1]), (char *)indirect);
            }
