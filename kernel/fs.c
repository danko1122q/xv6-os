...
// Hint untuk alokasi blok berikutnya guna meningkatkan spatial locality (kontiguitas)
...
static uint balloc(uint dev) {
    ...
    for (b = 0; b < sb.size; b += BPB) {
        // Spatial Locality: Mulai dari hint terakhir agar blok file cenderung berdekatan
        uint curr_block = sb.bmapstart + ((last_alloc_hint + b) / BPB) % (sb.size/BPB + 1);
        bp = bread(dev, curr_block);

        // Optimasi: Cek 32-bit sekaligus. Jika 0xFFFFFFFF, berarti 32 blok tersebut penuh.
        for (int i = 0; i < BSIZE; i += 4) {
            if (*(uint*)&bp->data[i] != 0xFFFFFFFF) {
                for (bi = (i * 8); bi < (i * 8) + 32; bi++) {
                    m = 1 << (bi % 8);
                    if ((bp->data[bi / 8] & m) == 0) {
                        bp->data[bi / 8] |= m; // Tandai digunakan
                        log_write(bp);
                        brelse(bp);
                        
                        uint bno = ((curr_block - sb.bmapstart) * BPB) + bi;
                        bzero(dev, bno);
                        last_alloc_hint = bno; 
                        return bno;
                    }
...
void iinit(int dev) {
    ...
    readsb(dev, &sb);
    cprintf("NorthOS FS: size %d, nblocks %d, inodestart %d, bmapstart %d\n",
...
struct inode *ialloc(uint dev, short type) {
    ...
    for (inum = 1; inum < sb.ninodes; inum++) {
        ...
        if (dip->data.type == 0) { // Menggunakan dip->data karena struktur union di fs.h
