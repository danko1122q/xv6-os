...
void renderAllApps() {
	for (int i = 0; i < MAX_APPS; i++) {
		...
		// Render text dengan width yang lebih besar untuk menampung nama panjang
		drawString(&desktop, apps[i].name, textColor, 
		          textX, labelY, 80, LABEL_HEIGHT);
	}
}
...
void customUpdateWindow() {
	...
	if (GUI_getMessage(desktop.handler, &msg) == 0) {
		...
		if (msg.msg_type == M_MOUSE_DOWN) {
			...
			if (appIdx != -1) {
				...
				if (appIdx == lastClickApp && 
					...
					// SINGLE CLICK - Store initial click position
					draggingApp = appIdx;
		...
		} else if (msg.msg_type == M_MOUSE_LEFT_CLICK || msg.msg_type == M_MOUSE_UP) {
			// Mouse UP/CLICK - stop drag
			if (draggingApp != -1) {
				if (!isDragging) {
					// Was a click, not a drag - check for start button
			...
			// Double click langsung - ignore, sudah dihandle di M_MOUSE_DOWN
			draggingApp = -1;
			isDragging = 0;
			lastClickApp = -1;
		}
		...
		for (int p = desktop.widgetlisthead; p != -1; p = desktop.widgets[p].next) {
			if (desktop.widgets[p].type == BUTTON) {
				...
				drawFillRect(&desktop, w->context.button->bg_color, 
				            w->position.xmin, w->position.ymin, width, height);
				drawRect(&desktop, black, w->position.xmin, w->position.ymin, 
				        width, height);
				drawString(&desktop, w->context.button->text, 
				          w->context.button->color,
				          w->position.xmin + textXOffset, 
				          w->position.ymin + textYOffset, width, height);
			}
		}
	}
}
